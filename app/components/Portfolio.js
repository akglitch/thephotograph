...
export default function Portfolio() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-indigo-950 via-purple-900 to-pink-900 text-white">
      <div className="relative mx-auto max-w-7xl px-6 py-24 sm:px-8">
        <div className="text-center space-y-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs uppercase tracking-widest text-white/80">
              ...
              <button
                ...
                className={`rounded-full border px-6 py-3 text-sm font-medium transition-all duration-300 ${
                  activeCategory === category.id
                    ...
                    : 'border-white/20 bg-white/10 text-white/80 hover:border-pink-300 hover:bg-white/20'
                }`}
              >
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {filteredImages.map((image, index) => (
            <article
              key={image.id}
              className="group relative overflow-hidden rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 transition-all duration-500 hover:border-pink-400/50 hover:bg-white/10"
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <p className="text-xs uppercase tracking-widest text-white/80 mb-2">{categoryLabels[image.category]}</p>
        ...
        <div className="mt-20 text-center">
          <div className="inline-block rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 p-8">
            <h2 className="text-3xl font-bold text-white mb-4">Ready to Create Your Story?</h2>
            <p className="text-white/70 mb-6">Let's collaborate on capturing moments that matter.</p>
            <button className="rounded-full bg-gradient-to-r from-pink-500 to-purple-500 px-8 py-4 text-white font-semibold hover:from-pink-600 hover:to-purple-600 transition-all duration-300 shadow-lg">
              Start Your Project
            </button>
          </div>
        </div>
